...
RSpec.describe NormalServiceCalculator, type: :service do
  ...
  let(:normal_service_calculator) { NormalServiceCalculator.new(beginning) }
  ...
  context 'when there are no company holidays during service' do
    ...
    describe '#calculate_chargeable_service_days' do
      ...
      context 'when ending is between 25 and 354 days after beginning' do
        ...
        let(:week_days_range) { (start_range..end_range).reject(&:on_weekend?) }
    ...
    describe '#calculate_eligible_personal_vacation_days' do
      subject { calculate_eligible_personal_vacation_days }

      let(:service_days) { 0 }
      let(:calculate_eligible_personal_vacation_days) do
      ...
      context 'when service days are between 0 and 179' do
        it 'returns 0', :aggregate_failures do
          (0..179).each do |days|
            personal_vacation_days = normal_service_calculator.calculate_eligible_personal_vacation_days(days)
